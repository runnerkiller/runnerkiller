...
**현재 알려진 문제**: Render에 올라간 Bridge가 켜지긴 했지만, `/health` 요청을
보내면 응답 없이 계속 멈추는 현상이 있다. 서버 프로세스 자체는 살아있는 것으로
보이므로(시작 로그 정상), Discord API를 호출하는 부분에서 뭔가 막히고 있을
가능성이 높다. 방금 원인 파악을 위한 요청 로그를 추가했다 — 다음 배포 후 Render
로그에 각 요청이 얼마나 걸렸는지 찍히니, 그걸 보면 정확한 원인을 알 수 있다.
이 문제가 해결되기 전까지는 사이트를 `discord` 모드로 전환하지 않는다.

## 다음 남은 일

1. `/health` 응답 지연 원인 확인 (로그 재확인)
2. 원인 해결 후 Bridge가 Discord와 실제로 통신되는지 확인
3. `runtime-config.js`를 `discord` 모드로 전환, Render 주소 입력
4. 실제로 제보를 하나 제출해보고 Discord 채널에 메시지가 생기는지, 사이트
   목록에도 뜨는지 끝까지 확인
