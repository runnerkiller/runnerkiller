...
두 가지 확인 경로가 있고 용도가 다르다.

- **`GET /livez`** — Discord를 전혀 부르지 않고 즉시 `{ "status": "ok" }`를 돌려준다.
  프로세스가 요청을 받을 수 있는 상태인지만 본다. **Render 같은 호스팅의 자체
  배포 성공 판정(health check)은 반드시 이 경로를 봐야 한다** — `render.yaml`도
  이렇게 설정돼 있다. `/health`를 여기 연결하면 Discord 응답이 느려질 때마다
  호스팅이 "배포 실패"로 오판해 트래픽 자체를 안 붙여줄 수 있다.
- **`GET /health`** — Discord 연결과 설정 읽기까지 실제로 확인한다. 사람이나
  모니터링 도구가 "지금 진짜로 정상 동작하는지" 보려고 쓴다. Discord가 느리거나
  응답이 없어도 **최대 8초 안에는 반드시 응답한다** (그 이상 걸리면 타임아웃
  오류로 답한다).
